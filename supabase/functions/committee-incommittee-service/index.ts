...
import { Bill, Committee, CommitteeReferral } from "../../shared/types.ts";
import { UpdateSchedule, UpdateScheduleDAO } from "../../shared/UpdateScheduleDAO.ts";

configure();
const parser = new XMLParser();

async function fetchDetail(committee: Committee, biennium: string): Promise<CommitteeReferral[]> {
  ...
  const members = json["ArrayOfLegislationInfo"]["LegislationInfo"];
  return members;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const entities = await dao.findLastUpdateBefore(sched.next_update, 'incommittee_update')
    ...
    for (let i = 0; i < entities.length; i++) {
      ...
      const updatedDBCommittee = {
        ...
        updated_at: now,
        incommittee_update: now,
      }
      await dao.upsert(updatedDBCommittee);
    }
    console.info(`Updated ${entities.length} committees with in-committee information.`);
